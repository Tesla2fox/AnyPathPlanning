...
{
	{
		std::cout << "maxCol = " << this->m_MaxCol << std::endl;
		std::cout << "maxRow = " << this->m_MaxRow << std::endl;


		svg::Dimensions dimensions(m_MaxCol * this->gridStep * 2, m_MaxRow * this->gridStep*2);
		...
		svg::Document doc("SMap.svg", svg::Layout(dimensions, svg::Layout::BottomLeft));
		...
		auto InitVp = this->Tgraph[0];
		double init_x = InitVp.pnt.x() - 3;
		double init_y = InitVp.pnt.y() - 3;
		...
		for (bgeo::VertexIterator vit = _b2e_vi.first; vit != _b2e_vi.second; vit++)
		{
			bgeo::VertexDescriptor vd = *vit;
			bgeo::VertexProperty &vp = this->Tgraph[vd];
			double px = (vp.pnt.x() - init_x) ;
			double py = (vp.pnt.y() - init_y) ;
			...
			case bgeo::vertType::ObVert:
			{
				doc << svg::Circle(pnt, 5, svg::Color::Blue);
				break;
			}
			case bgeo::vertType::EdgeObVert:
			{
		...
		for (auto eit = _b2e_ei.first; eit != _b2e_ei.second; eit++)
		{
			...
			double spx = (sVert.pnt.x() - init_x) ;
			double spy = (sVert.pnt.y() - init_y) ;
			//std::cout << "px" << px << std::endl;
			//std::cout << "py" << py << std::endl;
			svg::Point spnt(spx, spy);

			double tpx = (tVert.pnt.x() - init_x) ;
			double tpy = (tVert.pnt.y() - init_y) ;
			//std::cout << "px" << px << std::endl;
			//std::cout << "py" << py << std::endl;
	...
	{
		svg::Dimensions dimensions(10 * this->m_AMaxCol, 10 * this->m_AMaxCol);
		...
		for (auto &it : this->ATgrid)
		{
			...
			double px = (it.second.pnt.x() - init_x);
			double py = (it.second.pnt.y() - init_y);
			svg::Point pnt(px, py);

			//svg::Point pnt(it.second.pnt.x() - init_x, it.second.pnt.y() - init_y);
			switch (it.second.type)
			{
			case bgeo::vertType::ObVert:
			{
			...
			case bgeo::vertType::EdgeObVert:
			{
				doc << svg::Circle(pnt, 3, svg::Color::Red);
				break;
			}
			case bgeo::vertType::ShoulderVert:
			{
				doc << svg::Circle(pnt, 3, svg::Color::Brown);
				break;
			}
			case bgeo::vertType::NearVert:
			{
				doc << svg::Circle(pnt, 3, svg::Color::Fuchsia);
				break;
			}
			default:
				break;
	...
	//{
	//	svg::Dimensions dimensions(10 * this->m_AMaxCol, 10 * this->m_AMaxCol);
	//	svg::Document doc("GMap.svg", svg::Layout(dimensions, svg::Layout::BottomLeft));

	//	auto InitVp = this->GTgrid.at(ob::gridIndex(0, 0));

	//	double init_x = InitVp.pnt.x() - 3;
	//	double init_y = InitVp.pnt.y() - 3;


	//	for (auto &it : this->GTgrid)
	//	{

	//		double px = (it.second.pnt.x() - init_x) * 10;
	//		double py = (it.second.pnt.y() - init_y) * 10;
	//		svg::Point pnt(px, py);
